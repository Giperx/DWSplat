...
TARGET_HEIGHT = 294 
TARGET_WIDTH = 518
RENDER_H = 294      # Height for final rendering (if different from input)
WIDE_W = 1036        # Width for wide-angle rendering
...
SAVE_ROOT_BASE = "./renders_val_v2omni_v1weight_518"
if ENABLE_FUSION:
    ...
    for i in tqdm(range(num_batches), desc="Inference"):
        ...
        for task in batch_tasks:
            try:
                ...
                # v_exts = closed_form_inverse_se3(v_exts)
        ...
        with torch.no_grad():
            ...
            render_intrinsics = b_intrinsics.clone()
            ...
            # Forward Decoder
            # Note: Decoder needs EXTRINSICS. We use the ones from batch (context).
            outputs = model.decoder.forward(
                gaussians,
                b_extrinsics.inverse(), # pred_context_pose['extrinsic'],
                render_intrinsics, # render_intrinsics.float(),
                t_near,
                t_far,
