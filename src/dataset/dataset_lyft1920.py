...
@dataclass
class DatasetLyft1920Cfg(DatasetCfgCommon):
    """Configuration for Lyft 1920 dataset loader"""
    name: str
    roots_train: Any
    roots_val: Any
    baseline_min: float
...
@dataclass
class DatasetLyft1920CfgWrapper:
    lyft1920: DatasetLyft1920Cfg


class DatasetLyft1920(Dataset):
    """
    Lyft 1920 dataset loader.
    Structure:
        root/{scene_id}/
    ...
    def __init__(
        ...
        if split_name is None:
            split_name = "train1920.txt" if stage == "train" else "lyft_val1920.txt"
        ...
        logger.info(f"Lyft1920 Dataset: {self.stage}: loaded {len(self.samples)} samples from {len(self.scene_ids)} scenes")
    ...
    def _read_car_cam_mask(self, cam_id: int) -> torch.Tensor:
        ...
        if not file_path.exists():
            print(f"Car cam mask not found for cam_id {cam_id} at {file_path}, using all-ones mask.")
            return torch.ones((1, self.TARGET_HEIGHT, self.TARGET_WIDTH), dtype=torch.float32)
        image = Image.open(file_path).convert('L')
        image = image.resize((self.TARGET_WIDTH, self.TARGET_HEIGHT), Image.NEAREST)
        return self.mask_to_tensor(image)
