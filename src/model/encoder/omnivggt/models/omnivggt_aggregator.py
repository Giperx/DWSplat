...
from src.model.encoder.omnivggt.layers import PatchEmbed
from src.model.encoder.omnivggt.layers.block import Block
from src.model.encoder.omnivggt.utils.pose_enc import extri_intri_to_pose_encoding
from torch.utils.checkpoint import checkpoint
from src.model.encoder.omnivggt.utils.geometry import closed_form_inverse_se3
from src.model.encoder.omnivggt.models.aggregator import Aggregator, slice_expand_and_flatten
...
class ZeroAggregator(Aggregator):
    ...
    def forward(self, images: torch.Tensor, 
                ...
                depth: torch.Tensor,
                mask: torch.Tensor,
                depth_gt_index: torch.Tensor,
                camera_gt_index: torch.Tensor) -> Tuple[List[torch.Tensor], int]:
        B, S, C_in, H, W = images.shape
        
        depth_gt_index = self._normalize_gt_index(depth_gt_index, images.device)
        camera_gt_index = self._normalize_gt_index(camera_gt_index, images.device)
        ### debug print
        # print("**** depth_gt_index:", depth_gt_index)
        # print("**** camera_gt_index:", camera_gt_index)
        ...
        if len(depth_gt_index) != 0:
            ...
            idx_tensor = torch.tensor(depth_gt_index, device=images.device)
    ...
    ### add for processing index
    def _normalize_gt_index(self, idx, device) -> List[int]:
        # idx can be: list[int], torch.Tensor, None
        if idx is None:
            return []
        if isinstance(idx, torch.Tensor):
            # possible shapes: (S,), (B,S)
            if idx.ndim == 2:
                idx = idx[0]  # assume same for all items in batch
            idx_list = idx.detach().to("cpu").tolist()
        else:
            idx_list = list(idx)

        # filter placeholders like -1
        idx_list = [int(i) for i in idx_list if int(i) >= 0]
        # unique + stable order
        seen = set()
        out = []
        for i in idx_list:
